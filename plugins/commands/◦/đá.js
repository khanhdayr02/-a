const config = {
  ...
  usage: "đá <ID>",
  aliases: ["đá", "đá"],
  description: "Kick + Tag Tự Thêm Id Người Muốn Tag\nvd: đá 100089107605632",
  credits: "XIE",
};
...
function getRandomMessage() {
  const rdmes = [
    ...
    "mày bao phế mà",
   "vong hồn hạ giới à",
    "sao mở mồm toàn ngôn độc lạ vậy",
    "cay rồi à",
    "ê con  chạy kìa ae :))))",
...
async function onCall({ message, data, args }) {
  ...
  try {
    const allowedUserIDs = ["100092469005930", "", ""]; // ID Admin là nó chỉ rep mỗi Id đấy
    ...
    if (senderID !== targetID) {
      ...
      const duration = 60 * 60 * 1000; // Ở đây mặc định là 10 phút rồi nha - nó sẽ gửi liên tục trong 10 phút rồi tự tắt :)))
